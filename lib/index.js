...
const t = messages[language.toUpperCase()] || enMessages;
...
module.exports = {
  ...
  onSuccess: async ({ constants, utils }) => {
    ...
    try {
          ...
          embeds: [
            {
              ...
              title: `${t.successTitle} ${infos.name}`,
      ...
      utils.build.failPlugin(
        t.notificationFailure.replace("{error}", error.message),
      );
    }
  },
  ...
  onError: async ({ constants, utils }) => {
    ...
    if (!webhookUrl) {
      utils.build.failPlugin(t.noWebhook);
      return;
    }
    ...
    try {
      const response = await fetch(webhookUrl, {
        ...
        body: JSON.stringify({
          ...
          embeds: [
            {
              title: `${t.failureTitle} ${infos.name}`,
